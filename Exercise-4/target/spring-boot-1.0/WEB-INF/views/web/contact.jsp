...
<div class="page-wrapper">
    ...
    <div id="success-alert-container" style="position: fixed; top: 75px; right: 20px; z-index: 1050; width: 300px;">
...
<script>
    ...
    function addCustomer(data) {
        $.ajax({
            ...
            success: function (response) {
                window.location.href = "<c:url value='/lien-he?message=insert_success'/>";
                alert("Send your contact successfully")
            },
            error: function (response) {
                window.location.href = "<c:url value='/lien-he?message=insert_error'/>";
                alert("Error")
            }
        });
    }
    ...
    <c:if test="${param.message == 'insert_success'}">
    $(document).ready(function () {
        // Kích hoạt hàm hiển thị alert khi giao thành công
        showAlert("Thêm thông tin liên hệ thành công.");
    });
    </c:if>

    <c:if test="${param.message == 'insert_error'}">
    $(document).ready(function () {
        // Kích hoạt hàm hiển thị alert khi giao thành công
        showAlert("Thêm thông tin liên hệ thất bại.");
    });
    </c:if>
